...
public class LSDLiteral {
	...
	public LSDLiteral(LSDPredicate pred, List<LSDBinding> bindings,
		...
		;
		// MK we are going to add a preference in terms of which predicate variables must be constants. 
	...
	// MK what is the role of primarybinding? 
	...
	public LSDVariable getKeyPredicateVariable() {
		int key = this.predicate.getKeyArgument();
		if (key >= 0) {
			return this.bindings.get(key).getVariable();
		}
		return null;

	}
	public boolean isDesirableVariableToSubstituteWithConstants(LSDVariable v) {
		...
		// MK 020611: How can I force rule styles so that only certain arguments can be substituted with constants?  
	...
	public boolean atLeastOneConstHolder() {
		for (int i =0; i<bindings.size(); i++) {
			LSDBinding b = bindings.get(i);
			LSDVariable v = b.getVariable(); 
			if (v!=null && v.isConstantHolder()) return true; 
		}	
		return false; 
	}
	public boolean atLeastOneVariable() {
		for (int i =0; i<bindings.size(); i++) {
			LSDBinding b = bindings.get(i);
			LSDVariable v = b.getVariable(); 
			if (v!=null && v.isConstantHolder()==false) return true; 
		}	
		return false;
	}
	public boolean atLeastOneConstGrounding() {
		for (int i =0; i<bindings.size(); i++) {
			LSDBinding b = bindings.get(i);
			if (b.isBound()==true) return true; 
		}	
		return false;
	...
	public String toStringIgnoreConstantHolders() {
		String bs = "";
		for (int i = 0; i < bindings.size(); i++) {
			bs = bs + bindings.get(i).toStringIgnoreConstantHolders();
