...
public class LSDRule implements Comparable<LSDRule>{
	...
	// MK 020611. this method checks that there's at least one variable shared between literals 
...
//		// MK 020811 I am adding another constraint that every literal must have at least one constant holder and at least one variable
		if (RestrictionOption.OPTION_AtLeastOneConstantHolderOneVariablePerPredicate) {
			for (LSDLiteral literal : this.literals) {
				if (literal.atLeastOneConstHolder() == false
						&& literal.atLeastOneVariable() == false) {
					return false;
				}
			}
		}
	...
	public String toStringIgnoreConstantHolders() { 
		String output = "";
		for (LSDLiteral literal : literals) {
			output += literal.toStringIgnoreConstantHolders();
		}
		return output;
	}
	...
	public int hashCode() {
		return toStringIgnoreConstantHolders().hashCode();
	}
	...
	public LSDVariable getKeyConclusionVariable() {
		// find a conclusion literal
		if (getConclusions() == null)
			return null;
		LSDLiteral conclusion = getConclusions().getLiterals().get(0);
		return conclusion.getKeyPredicateVariable();
	}

	public boolean hasConstantsInEveryAntecendent() { 
		LSDRule antecedents = this.getAntecedents();		
		for (LSDLiteral literal: antecedents.literals) { 
			if (!literal.atLeastOneConstGrounding()) { 
				return false;  
			}
 		}
		return true;
	...
	@Override
	public int compareTo(LSDRule o) {
		return this.hashCode()-o.hashCode() ;
	}

//	public boolean sameIgnoreConstantHolders(LSDRule ur2) {
//		String ur1_ignoreConstantHolders = toStringIgnoreConstantHolders();
//		String ur2_ignoreConstantHolders = ur2.toStringIgnoreConstantHolders();
//		return (ur1_ignoreConstantHolders.hashCode() == ur2_ignoreConstantHolders
//				.hashCode());
//	}
